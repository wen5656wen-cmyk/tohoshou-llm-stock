...
export default function MissionAuditPage() {
  ...
  useEffect(() => {
    // P21-T5-4B：改用 S1 建立的 httpOnly 会话 Cookie（同源请求自动携带）。
    // 原实现读 localStorage 的 llmstock_admin_token —— S1 已移除该机制，故恒 401。
    fetch("/api/admin/mission-audit", { credentials: "same-origin" })
      .then((r) => r.json())
