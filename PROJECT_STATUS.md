...
| DailyRecommendation | **1,622+（4日期）** | ✅ 2026-06-24:500(today)，2026-06-23:550⚠️(异常>500)，2026-06-20:500，2026-06-22:72(补填) |
...
| P2 | DailyRecommendation 2026-06-23 = 550条 | 正常上限500，疑似当日两次 pipeline 运行导致重复 upsert 问题；需排查 @@unique([date, symbol]) 是否生效 |
...
| 异常数据 | 2026-06-23 DailyRec=550（>500）| psql |
...
2. **DailyRecommendation 2026-06-23=550**：查原因（`SELECT date, COUNT(*) FROM daily_recommendations WHERE date='2026-06-23' GROUP BY date`），可能是当日两次 pipeline
...
#### 选项 A（P2 修复）：GPT rerank + DailyRec 550 异常
- 生产运行 `rerank:top500` 补全 283 个 nullRank
- 排查 2026-06-23=550 异常原因
