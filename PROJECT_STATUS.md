...
> **最后更新：** 2026-06-21
> **版本：** v8.3（全局UX统一 — lib/rec-config.ts + 绿涨红跌 + rounded-2xl + 英文标签，已部署生产）
...
### 当前状态（2026-06-21 会话结束时）
- **v8.3 P1 已完成并部署**（commit 61ebe8d）
- 所有 11 个 UI 文件已统一设计规范
- 生产 PM2 已重启并在线
...
#### 选项 A：v8.3 P2 — 页面功能增强
可考虑以下增强：
- `/stocks/[symbol]` — 雷达图标签英文化（当前 RadarChart labels 仍为中文：技術面/基本面/資金面/情绪/趋势）
- `/ai-theme/[theme]` — 供应链流可视化改进（当前 flex+SVG 方案）
- `/chat` — Web 聊天页 UX 改善（打字机效果、历史记录）
- 添加 `/portfolio` 实际功能（当前仅有 Schema，页面为空）

#### 选项 B：v8.4 — 新功能
- **LINE Bot 增强** — 支持更多意图类型（sector_outlook/stock_compare 中文回答）
- **数据源增强** — 更多 GlobalMarket 指标（恐惧贪婪指数、美债收益率）
- **个股页增强** — 新闻情绪时间轴、同行业对比

#### 选项 C：维护/修复
- 运行 `npm run audit:data` 查看最新审计结果
- 运行 `npm run health:data` 查看数据健康状态
- 检查生产 PM2 日志：`pm2 logs tohoshou-web --lines 50`
