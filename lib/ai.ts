...
export async function generateDailyPicks(
...
): Promise<{ symbol: string; rank: number; reason: string }[]> {
  // Sort by aiScore
  const ranked = [...stocks]
  ...
  if (!process.env.OPENAI_API_KEY) {
    return ranked.map((s, i) => ({
      symbol: s.symbol,
      rank: i + 1,
      reason: `${s.name}はAIスコア${s.aiScore}点で上位評価。${s.sector || "主要セクター"}において競争優位を持ち、${s.per ? "PER" + s.per.toFixed(1) + "倍の" : ""}${(s.roe || 0) > 12 ? "高ROE" : "安定した"}収益性が評価されます。`,
    }));
  }

  const modelName = process.env.AI_MODEL || "gpt-4o-mini";
  const list = ranked
    .map((s) => `${s.symbol} ${s.name}: スコア${s.aiScore}, PER=${s.per?.toFixed(1) || "N/A"}, ROE=${s.roe?.toFixed(1) || "N/A"}%`)
    .join("\n");
  ...
  try {
    const resp = await openai.chat.completions.create({
      ...
      messages: [{
        role: "user",
        content: `本日の日本株推奨銘柄TOP5を以下から選定し、各銘柄の投資理由を100字で説明してください。\n${list}\n\nJSON形式: {"picks":[{"symbol":"コード","rank":1,"reason":"理由"},...]}`,
      }],
      response_format: { type: "json_object" },
      temperature: 0.8,
    });
    const r = JSON.parse(resp.choices[0].message.content || "{}");
    return r.picks || ranked.map((s, i) => ({ symbol: s.symbol, rank: i + 1, reason: "" }));
  } catch {
    return ranked.map((s, i) => ({ symbol: s.symbol, rank: i + 1, reason: "" }));
  }
}
