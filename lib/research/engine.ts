// ── Deep Research · Research Engine（P17 Phase 3 核心 / V2 候选流程）──────────
...
// V2 流程：候选以 payload 入库 AI_RESEARCHED（不即时改写实体，V1 完整）→ Review Approve → applyVersion 才落地。
...
export interface ApplyCounts { segments: number; technologies: number; companies: number; claims: number; evidence: number; edges: number; bottlenecks: number; stockLinks: number; hiddenChampions: number }

// 落地 payload 到实体（幂等）。runIndustryResearch 与 applyVersion(approve) 复用。
// ⚠️ 会 delete-recreate 该产业的 claims/edges/bottlenecks —— 仅在首版 或 V2 获批时调用。
export async function applyPayload(R: IndustryResearch): Promise<{ indId: string; counts: ApplyCounts }> {
  const now = new Date();
  ...
  // 4) Companies + links + HC + StockLink + claims
  ...
  return { indId: ind.id, counts: { segments: R.segments.length, technologies: R.technologies.length, companies: R.companies.length, claims: claimCount, evidence: evidenceCount, edges: edgeCount, bottlenecks: R.bottlenecks.length, stockLinks, hiddenChampions } };
}

// payload 内 claim/evidence 计数（候选未落库时用）
export function payloadEvidenceCounts(R: IndustryResearch): { claims: number; evidence: number } {
  let claims = 0, evidence = 0;
  const add = (arr?: { claims?: ClaimInput[] }[]) => { for (const x of arr ?? []) for (const c of x.claims ?? []) { claims++; evidence += c.evidence?.length ?? 0; } };
  add(R.companies); add(R.technologies); add(R.bottlenecks);
  return { claims, evidence };
}

export interface RunSummary {
  industryKey: string; versionId: string; version: string; jobId: string;
  counts: ApplyCounts; published: boolean; sourceKind: string;
}

// 首版 / seed：立即 apply + 建版本。seed→PUBLISHED；LLM→AI_RESEARCHED(待审)。
export async function runIndustryResearch(provider: ResearchProvider, industryKey: string): Promise<RunSummary> {
  ...
  try {
    ...
    const R = result.data;
    const now = new Date();
    const { indId, counts } = await applyPayload(R);

    const prev = await prisma.researchVersion.findFirst({ where: { entityType: "INDUSTRY", entityId: indId }, orderBy: { generatedAt: "desc" } });
    ...
    const ver = await prisma.researchVersion.create({
      data: {
        entityType: "INDUSTRY", entityId: indId, version, payload: R as object,
        status: isSeed ? "PUBLISHED" : "AI_RESEARCHED",
        changeReason: prev ? "重新深研" : "首版深研", previousVersionId: prev?.id ?? null,
        ...
        evidenceCount: counts.evidence, reviewStatus: isSeed ? "APPROVED" : "PENDING",
        reviewer: isSeed ? "seed-verified" : null, reviewedAt: isSeed ? now : null, publishedAt: isSeed ? now : null,
      },
    });
    if (isSeed) await prisma.researchReview.create({ data: { versionId: ver.id, reviewer: "seed-verified", action: "APPROVE", comment: "Golden Path 人工核验种子数据（真实公开来源）" } });
    await prisma.researchIndustry.update({ where: { id: indId }, data: { currentVer: version, status: isSeed ? "PUBLISHED" : "AI_RESEARCHED" } });
    ...
    await prisma.researchJob.update({ where: { id: job.id }, data: { status: "SUCCESS", model: result.usage.model, tokenUsage: { prompt: result.usage.promptTokens, completion: result.usage.completionTokens, total: result.usage.totalTokens }, estimatedCost: result.usage.estimatedCost, durationMs: Date.now() - t0, versionId: ver.id, finishedAt: new Date() } });

    return { industryKey: R.industry.industryKey, versionId: ver.id, version, jobId: job.id, counts, published: isSeed, sourceKind: result.sourceKind };
  } catch (err) {
...
export interface CandidateSummary { versionId: string; version: string; industryKey: string; claims: number; evidence: number; provider: string; model: string; estimatedCost: number; durationMs: number }

// V2 候选：仅把 payload 入库为新版本 AI_RESEARCHED（**不 apply 实体**，V1 完整保留）。供 Benchmark→Review。
export async function generateCandidateVersion(result: ResearchResult): Promise<CandidateSummary> {
  const R = result.data;
  const ind = await prisma.researchIndustry.findUnique({ where: { industryKey: R.industry.industryKey } });
  if (!ind) throw new Error(`产业 ${R.industry.industryKey} 不存在（候选需在已发布产业上生成）`);
  const prev = await prisma.researchVersion.findFirst({ where: { entityType: "INDUSTRY", entityId: ind.id }, orderBy: { generatedAt: "desc" } });
  const nextNum = prev ? parseInt(prev.version.replace(/\D/g, "")) + 1 : 1;
  const version = `V${nextNum}`;
  const ec = payloadEvidenceCounts(R);
  const ver = await prisma.researchVersion.create({
    data: {
      entityType: "INDUSTRY", entityId: ind.id, version, payload: R as object, status: "AI_RESEARCHED",
      changeReason: "Strong Model 重跑（候选，待审）", previousVersionId: prev?.id ?? null,
      provider: result.usage.provider, model: result.usage.model, promptVersion: "p17-v2",
      tokenUsage: { prompt: result.usage.promptTokens, completion: result.usage.completionTokens, total: result.usage.totalTokens },
      estimatedCost: result.usage.estimatedCost, durationMs: result.usage.durationMs, evidenceCount: ec.evidence, reviewStatus: "PENDING",
    },
  });
  await prisma.researchReport.create({ data: { scope: "INDUSTRY", refKey: R.industry.industryKey, title: `${R.industry.nameZh} · 深度研究 ${version}（候选）`, version, summary: R.industry.summary ?? null, content: { industryKey: R.industry.industryKey, candidate: true }, model: result.usage.model, status: "DRAFT" } });
  return { versionId: ver.id, version, industryKey: R.industry.industryKey, claims: ec.claims, evidence: ec.evidence, provider: result.usage.provider, model: result.usage.model, estimatedCost: result.usage.estimatedCost, durationMs: result.usage.durationMs };
}

// 审核通过时落地候选版本 payload 到实体，并置 PUBLISHED（V1 版本记录保留）。
export async function applyVersion(versionId: string): Promise<{ applied: boolean; version: string; counts?: ApplyCounts }> {
  const ver = await prisma.researchVersion.findUnique({ where: { id: versionId } });
  if (!ver || ver.entityType !== "INDUSTRY" || !ver.payload) return { applied: false, version: ver?.version ?? "" };
  const R = ver.payload as unknown as IndustryResearch;
  const { indId, counts } = await applyPayload(R);
  const now = new Date();
  await prisma.researchVersion.update({ where: { id: versionId }, data: { status: "PUBLISHED", publishedAt: ver.publishedAt ?? now } });
  await prisma.researchIndustry.update({ where: { id: indId }, data: { currentVer: ver.version, status: "PUBLISHED" } });
  await prisma.researchReport.updateMany({ where: { refKey: R.industry.industryKey, version: ver.version }, data: { status: "PUBLISHED", publishedAt: now } });
  return { applied: true, version: ver.version, counts };
}
