// ── Deep Research · Provider Adapter（P17 Track 2）──────────────────────────
// 正式多 provider 适配层：OpenAI / Anthropic / Seed。统一能力：
//   可用性检查 · 超时 · 重试 · 结构化输出校验 · 用量 · 成本 · 时长 · 原始响应审计 · 优雅降级 · 任务失败隔离
// ⚠️ 不修改 lib/openai.ts（仅复用其 client）。禁止写死模型名/密钥；密钥仅从环境读取，绝不入日志。
// ⚠️ LLM 产出=AI_RESEARCHED，须经证据绑定+人审才 PUBLISHED（防幻觉）。弱模型禁批量生成正式版本。
...
// ── 成本表（USD / 1K tokens）。仅估算；真实计费以账单为准。可扩展，不写死单一模型。 ──
...
// ── 深研 system prompt（provider 无关，保证 benchmark 公平）──
...
// ── 结构化输出校验（provider 无关，同一 schema/审核口径）──
export interface ValidationReport {
  schemaValid: boolean;
  errors: string[];
  warnings: string[];
  stats: {
    segments: number; technologies: number; companies: number; jpListed: number;
    bottlenecks: number; edges: number; claims: number; evidence: number;
    symbolErrors: number;            // listed 但 symbol 非 NNNN.T
    noEvidenceCertainClaims: number; // confidence=HIGH/MID 却无证据（应为 LOW）
    duplicateEdges: number;          // from|to|type 重复
  };
}
...
export function validateIndustryResearch(data: unknown): ValidationReport {
  ...
  const countClaims = (claims: unknown) => {
    for (const cl of arr<Record<string, unknown>>(claims)) {
      s.claims++;
      const ev = arr<unknown>(cl.evidence); s.evidence += ev.length;
      const conf = String(cl.confidence ?? "").toUpperCase();
      if ((conf === "HIGH" || conf === "MID") && ev.length === 0) s.noEvidenceCertainClaims++;
    }
  };
  for (const c of cos) {
    const listed = c.listed === true;
    const sym = typeof c.symbol === "string" ? c.symbol : null;
    if (listed) { s.jpListed++; if (!sym || !SYM_RE.test(sym)) s.symbolErrors++; }
    countClaims(c.claims);
  }
  ...
  for (const e of edges) {
    const k = `${e.fromKey}|${e.toKey}|${e.edgeType}`;
    if (seen.has(k)) s.duplicateEdges++; else seen.add(k);
  }
...
export interface ProviderRunResult extends ResearchResult {
  raw: string;                 // 原始响应（审计）
  validation: ValidationReport;
  attempts: number;
  fallbackUsed?: string;       // 若发生降级，记录实际使用的 provider:model
}
export interface StrongProvider extends ResearchProvider {
  kind: ProviderKind;
  model: string | null;
  checkAvailability(): Promise<AvailabilityReport>;
  run(industryKey: string, opts?: { sourcePack?: string; timeoutMs?: number; retries?: number }): Promise<ProviderRunResult>;
}
...
export class OpenAIProvider implements StrongProvider {
  ...
  async checkAvailability(): Promise<AvailabilityReport> {
    if (!process.env.OPENAI_API_KEY) return { available: false, kind: this.kind, model: this.model, reason: "OPENAI_API_KEY 未配置（.env）" };
  ...
  async run(industryKey: string, opts?: { sourcePack?: string; timeoutMs?: number; retries?: number }): Promise<ProviderRunResult> {
    if (!this.model) throw new Error("OpenAIProvider: 无模型（配置 RESEARCH_MODEL）");
    const model = this.model, t0 = Date.now();
    const { value, attempts } = await withRetry(async () => {
      return openaiClient().chat.completions.create({ model, messages: [{ role: "system", content: RESEARCH_SYSTEM }, { role: "user", content: buildUserPrompt(industryKey, opts?.sourcePack) }], response_format: { type: "json_object" }, temperature: 0.2 });
    ...
    return { data, raw, validation: validateIndustryResearch(data), attempts, sourceKind: "LLM", usage: { provider: this.name, model, promptTokens: pt, completionTokens: ct, totalTokens: pt + ct, estimatedCost: estimateCost(model, pt, ct), durationMs: Date.now() - t0 } };
  }
...
// ── Anthropic provider（强模型；需 ANTHROPIC_API_KEY + RESEARCH_STRONG_MODEL）──
// web_search 取真证据 + adaptive thinking + 严格 JSON（自有 validator 校验，跨 provider 公平）。
export class AnthropicProvider implements StrongProvider {
  kind: ProviderKind = "anthropic";
  name = "anthropic";
  model: string | null;
  constructor(model?: string) { this.model = model ?? process.env.RESEARCH_STRONG_MODEL ?? null; }
  private client() {
    // 动态引入，避免未装 SDK 时打包报错；密钥仅从环境读取，绝不打印。
    // eslint-disable-next-line @typescript-eslint/no-require-imports
    const Anthropic = require("@anthropic-ai/sdk").default;
    return new Anthropic({ apiKey: process.env.ANTHROPIC_API_KEY });
  }
  async checkAvailability(): Promise<AvailabilityReport> {
    if (!process.env.ANTHROPIC_API_KEY) return { available: false, kind: this.kind, model: this.model, reason: "ANTHROPIC_API_KEY 未配置（应在服务器 /opt/tohoshou/.env 配置）" };
  ...
  async run(industryKey: string, opts?: { sourcePack?: string; timeoutMs?: number; retries?: number }): Promise<ProviderRunResult> {
    if (!this.model) throw new Error("AnthropicProvider: 无模型（配置 RESEARCH_STRONG_MODEL）");
    const model = this.model, t0 = Date.now(), client = this.client();
    const { value, attempts } = await withRetry(async () => {
...
// ── Seed provider（人工核验；产出 sourceKind=SEED → 直接可 PUBLISHED）──
...
export class SeedProvider implements StrongProvider {
  ...
  async run(industryKey: string): Promise<ProviderRunResult> {
    ...
    return { data, raw: JSON.stringify(data), validation: validateIndustryResearch(data), attempts: 1, sourceKind: "SEED", usage: { provider: this.name, model: "seed", promptTokens: 0, completionTokens: 0, totalTokens: 0, estimatedCost: 0, durationMs: Date.now() - t0 } };
  }
  async research(industryKey: string): Promise<ResearchResult> { const r = await this.run(industryKey); return { data: r.data, usage: r.usage, sourceKind: r.sourceKind }; }
}

// ── 工厂：env 驱动，禁写死。role 决定取哪个模型环境变量。──
...
// ── 优雅降级 + 任务失败隔离：strong 失败时降级到 fallback，单产业失败不影响其它。──
export async function runWithFallback(industryKey: string, opts?: { primary?: StrongProvider; fallback?: StrongProvider; sourcePack?: string }): Promise<ProviderRunResult> {
  ...
  try { const r = await primary.run(industryKey, { sourcePack: opts?.sourcePack }); return r; }
  catch (e) {
