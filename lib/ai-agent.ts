...
async function buildAnalysisReply(code: string): Promise<string> {
  ...
  const ctx = [
    ...
    `AI综合评分：${score.totalScore}分（技术${score.technicalScore}，基本面${score.fundamentalScore}，安全性${score.riskScore}）`,
  ...
  const lines = [
    ...
    `技术：${score.technicalScore ?? "—"}  基本面：${score.fundamentalScore ?? "—"}  安全性：${score.riskScore ?? "—"}`,
    `推荐：${recLabel(score.recommendation)}`,
    ``,
    `━━━━━━━━━━━━`,
    `💹 技术面`,
