...
### 🔴 阻断级修复：i18n 中英混排（P19-T2 引入）
违反 `docs/DESIGN_FIRST_GOVERNANCE.md` 明文规定「语言切换整页 100% 同语言」：
- `br.asOf.close` 「收盘 as of」→ **「收盘 · 截至」**
- `br.asOf.today` 「as of」（中文界面显示纯英文）→ **「截至」**
- `br.asOf.audit` 「审计 as of」→ **「审计 · 截至」**
- `DecisionStrategyV2.tsx` 硬编码 `as of {asOf}` → 新增 `br.asOf.plain` 走 i18n
- 页脚 `br.foot.asOf` 中的技术名词 "As Of" → 中文化为「数据基准时间」
- ja 侧同步为 「引け · 基準 / 基準 / 監査 · 基準」

**责任说明**：该问题由 P19-T2 引入，且在 T2 验收中被报告为「i18n ✅」—— 当时的自动检查只验证了「日文页无中文残留」，**未检查「中文页出现英文」**，属验证不完整导致的误报。本次已补全双向检查。

### 📋 IA Review 结论（只读审计，源码扫描 + 生产实测）
- **职责矩阵**：7 页各答唯一问题，无重复职责
- **重复统计**：✅ 无。业绩统计已全收敛至战绩档案；Overview 底部四栏深度统计 `grep` 零命中；Briefing 零业绩关键词
- **重复计算**：✅ 无第二套收益/Alpha 计算（⚠️ 残留 1 处：`DecisionOverviewV2.tsx:124` 账户超额前端自算，属账户总览摘要、口径独立，列入建议）
- **跳转链路**：✅ 生产实测 **10/10 HTTP 200，零死链**；无自动循环；多入口均上下文相关
- **链路缺口**（非死链）：Overview 无「今日简报」直达入口；Mission Lab 为零出口叶子节点
- **时间口径**：6 类语义梳理完毕；As Of 措辞原有 7 套，本次修正 4 处后中英混排归零

### 后续建议（不在本轮开发）
P1 统一 As Of 为单一 i18n 组 · P1 修复 `mission-control` 的 `todayPipeline` 状态源（实测 `compute_scores` 显示 2026-07-04 而真实 07-21）· P2 补 Overview→简报入口 / Mission Lab 出口 / Overview:124 改由 API 提供 · P3 Overview 6 请求合并 / P19-X 财报除权数据源 / W29 周滚动验证。

### 边界
零 Schema · 零评分 · 零交易 · 零 Engine · 零 Cron。tsc 0 / build ✅ / 五页 × 双语 i18n 纯净度 **10/10 通过**。
...
- **研究库**：版本表列头汉化(实体/版本/状态/提供方/证据)+字号微调。+5 dr.lib.col* 双语键。
- **审核中心/运营看板**：沿用已本地化标签 + 弹窗 Portal 居中修复(见 18.41.4)。
- 纯 UI；build✅/tsc0。
...
- `nav-config.ts` 给 `dv-overview` / `dv-portfolio` 设 `badge:"dv.nav.badge.core"`（复用现有机制，无组件改动）。纯导航配置。tsc 0 / build ✅ / health CRITICAL=0 / 页面 200。

---

## [18.31.2] - 2026-07-19 — ✨ 深度研究加「核心」醒目徽章

- 「深度研究」是核心功能 → 侧栏 + 移动抽屉给它加渐变紫→蓝「核心/コア」小徽章（渐变 + 轻微光晕，紧跟标签）。
- `NavNode.badge` 字段本已存在但未渲染 → `Sidebar.tsx` + `MobileDrawer.tsx` 补渲染（`t(node.badge)`）；`dv-industry` 设 `badge:"dv.nav.badge.core"`；i18n `dv.nav.badge.core` = 核心/コア（zh+ja+types）。纯导航视觉，未改页面/数据/API。tsc 0 / build ✅ / health CRITICAL=0 / 页面 200。

---

## [18.31.1] - 2026-07-19 — 🧭 产业研究 → 深度研究（改名 + 移到第四位）

- 决策工作区导航：决策总览 → 股票中心 → 行业分析 → **深度研究** → 今日策略 → 模拟持仓 → 历史决策。
- `nav-config.ts`：`dv-industry` 从 #7 上移到 #4（行业分析之后），即 AI 产业链深研模块入口。
- `dv.nav.industry`：产业研究/産業研究 → **深度研究 / 深掘りリサーチ**（zh+ja）。纯导航文案+顺序，未改页面/数据/API。tsc 0 / build ✅ / health CRITICAL=0 / 页面 200。
...
- `nav-config.ts` 将 `dv-sectors` 从 #6 上移到 #3（选股 → 看行业轮动 → 再挑股，与股票中心衔接）。纯导航顺序，未改页面/数据/API。tsc 0 / build ✅ / health CRITICAL=0 / 页面 200。

---

## [18.30.1] - 2026-07-19 — 🧹 Stock Center 精修：删收盘决策状态带 + 加入按钮改星标

- **删收盘决策绿色状态带**：`VerdictBand`（顶部 verdict 绿条）从股票中心移除——择时/组合面（BUY_TODAY 等）已在「决策总览」承载，股票中心专注选股，避免重复。删除组件与渲染（`verdict` 字段仍在 API/Resp 类型中，未动接口）。
- **加入 → ⭐ 星标**：AI 执行榜 / 全市场 / 持仓·等待·观察 各列表的「加入 / ✓已加入」按钮统一换为星标切换 `FavStar`（★ 金色=已加入 / ☆ 灰=未加入，点击 toggle）；更轻、更符合股票工作台习惯。右栏「加入自选」主按钮保留文字。
- 纯 UI，未改数据/API/评分。tsc 0 / build ✅ / 生产 health CRITICAL=0 / 页面 200。仅重启 web。
...
- **修「已加入仍显加入」bug**：原「加入」按钮状态是各视图**本地乐观 Set**，已在自选里的股票仍显「加入」、切视图/刷新即丢。改为 **Shell 统一读取真实 watchlist 成员**（`favSet`，挂载时 `GET /api/watchlist`）→ 下发 AI 推荐 / 全市场；按钮据实显示 **加入 / ✓已加入**，且**点击可 toggle**（已加入再点=移除，`POST`/`DELETE` 同步）。自选视图移除后回调 `onChanged` 刷新 `favSet`，三视图状态一致。
- **新增「今日涨跌统计」**：AI 买入执行榜表格上方新增一条统计——今日 **▲上涨 N / ▼下跌 M / – 平盘 K · 平均 ±X%**（基于 Top10 的 `todayChangePct`，实时口径）。i18n 新增 `dv.sc.br.title/avg` + `dv.sc.added`（zh+ja+types）。
- **今日涨跌口径澄清**（回答用户）：**AI 推荐**「今日」= Yahoo 实时（`recommendations` 路由 `cur/prev-1`，`sourceStatus.quote=live` 时实时 ~15s，否则回退收盘快照）；**自选**「今日」= `RealtimeMarket`（准实时）；**全市场**= 5 日快照（非实时，用户确认保留）。
- 验收：tsc 0 / build ✅ / 生产 health CRITICAL=0 / 页面 200 / bundle 含新文案（已加入 / 今日涨跌）。仅重启 web。
...
- **候选可折叠**：`OpportunityTable` 标题栏本身即折叠开关（▾/▸，零多余头部）。机会=常显（可买入=最高优先）、等待=可折叠默认展开、**观察=可折叠默认收起**（纯监控、优先级最低）→ 行动类内容（持仓/机会/等待）主导视线。
- **布局回归稳态**：分析卡（组合健康度/AI超额/AI表现/学习状态）改为**底部全宽战绩页脚**，右栏回归紧凑情境侧栏（风险/市场/系统）。页脚不依赖两栏高度，观察折叠后也不产生错位留白，永远像刻意的页脚。
- **B 精华已内含**：淡季「AI 今日建议」由 DecisionBar 现有 instruction 承载，未叠加冗余文案。
- 验收：zh/ja 桌面 + 手机(390) 无横向溢出；tsc 0 / build ✅ / health CRITICAL=0。

---

## [18.24.0] - 2026-07-19 — 🎛️ Decision Center 布局提质（顶部去重 + 两栏平衡）

针对自评点名的两大短板做结构性优化。纯 UI 重排，未改数据/引擎/评分/API。

- **顶部去重（消除 4 层叠 buff）**：决策总览 tab 隐藏跨页共享的 `DecisionContextBar`（今日可建仓/市场状态/风险/建议仓位/AI信心）——其字段是页内 `DecisionBar` 的子集，重复展示。隐藏后顶部为 3 条各司其职、指标不重复：命令条(标题+搜索+休市) → 账户总览(我的组合) → 今日决策(DecisionBar)。其余 4 个 tab 仍保留 ContextBar。
- **两栏平衡（消除右下大片留白）**：把底部孤立的「组合健康度/AI表现/AI超额/学习状态」四栏并入右栏，形成**左=决策流（持仓→机会→等待→观察→历史）· 右=情境+战绩（风险→市场→健康度→超额→表现→学习→系统状态）**的专业终端布局；去掉 sticky 让高右栏自然流动，与左侧等高。
- **验收**：zh/ja 桌面 + 手机(390) 无横向溢出；tsc 0 / build ✅ / health CRITICAL=0。
...
- 新增 `fmtBigYen(v, lang)`：大额自动折算 **億/兆**（≥1兆→兆，≥1億→億，<1億→万），**单位随语言**（ja「億/兆」，zh「亿/万亿」）。
- 应用于基本面概览 + `FinTable`（历年财务）的营收/净利。
- 效果：KDDI 营收 `5,080,353,000,000` → **5.08万亿 / 5.08兆**；净利 → **6,177亿 / 6,177億**；小盘 4424 营收 → **18.9亿 / 18.9億**。
- ROE 显 `—` 为数据本身 null（诚实降级，非显示 bug，未改）。EPS 保持原值。
- 验收：zh 大盘(兆级) + ja 小盘(億级) 截图确认；tsc 0 / eslint 0 / build ✅ / health CRITICAL=0。
...
### 今日总览（重写）
删除旧 7 段重复内容 → 4 区：①今日决策(verdict+市场状态+今日机会数+机会分拆解) ②第一推荐(买入区间/T1·T2/止损/预计收益率/盈亏比/持仓期/≤3条真实理由) ③建议组合(仓位条+买区+T1+SL) ④今日回避(0~3 动态)。**已删除「现金比例」**（Portfolio Builder 固定归一 100%，恒为 0，无信息量）。
**诚实性护栏**：第一推荐标注「单一最优标的」、建议组合标注「分散配置方案（行业≤2）」；top1 未入组合时如实说明；组合成分若 `top10.inBuyZone=false` 打「现价已脱离买区，建议等回落」徽章。**禁止隐藏矛盾/伪造一致性。**
**回避规则**：先排除 组合∪第一推荐（组合优先）→ 按严重度(利空>风险>放量下跌>脱离买区)排序 → 最多 3 只；冲突时剔除并 `console.warn`；`recommended ∩ avoid` 断言为空；0 只时显示「今日暂无明确回避标的」，**绝不凑数**。

### 历史验证（重写）
表格：日期/决策/第一推荐/组合/一周收益/★ + 点击展开(AI分/GPT分/买入价/买区/目标/止损/持仓期/实际最高价/最终收益/是否达标/客观复盘)。
**价格口径（统一复权·拆股安全·杜绝前视偏差）**：基准=推荐日 `adjClose_D`；一周收益=`adjClose_{D+5}/adjClose_D−1`；实际最高价=`max(high×adjClose/close)` over (D,D+5]；目标/止损换算到复权尺度再比较；**只用 D 之后已发生的 bar**。**交易日不足 5 → 显示「验证中」，绝不提前判定成败。**

### AI 市场驾驶舱（重写）
**删除全部开发者指标**（Integrity/Cron/Health/Pipeline/Promotion/Shadow/Deployment/Build/Version）→ 市场状态 · 主题动能 · 产业链热度 TOP10 · 今日催化剂(TDnet 真实披露) · 风险提示 TOP5 · 今日热点主题 TOP5。
**诚实口径**：「主题动能」= 成分股 5日/20日涨幅均值（真实价格数据），**明确标注非机构资金流**；库中无按主题的机构资金流数据源（InstitutionalFlow 仅市场级）→ 不提供「资金流向」，**绝不用评分值伪装**。任一区块无可靠数据 → 「暂无可靠数据」。

### 导航
Tab 重排为 ①今日决策 ②AI Top Picks ③每日关注池 ④收盘决策 ⑤历史验证 ⑥AI市场驾驶舱（仅重排，无新增导航/Hub/Workspace，旧 `?tab=` 深链全可用）。

### 验收
Build ✅ tsc 0 · Health ✅ **CRITICAL=0** · 六 Tab 全 **200** · 驾驶舱 4 个依赖 API 全 200 · 开发者指标残留 **0**（仅注释中的“已移除”说明）· 前视偏差检查通过（今日 6 条历史全部正确显示「验证中」，最早 07-10 仅 4/5 交易日）。
...
### 首日自动运行验收（2026-07-11 · ⏳ PENDING，待 07-13 复验）
只读验收（未改代码）。**07-10 16:09 部署（当天 15:15 已过未触发）→ 07-11/07-12 周末（JPX 守卫实测正确判定 Weekend 跳过）→ 首个真正的自动交易日运行 = 07-13（周一）15:15**，故"自动触发链路"今日客观上尚未发生（不判 FAIL）。**就绪度全部 PASS**：守卫逻辑正确（dry-run 输出「07-11 非 JPX 交易日 → 跳过」）；07-10 首跑实测 12.3s / 实时 150/150 / GPT `calls=1 ok=1 fail=0 err429=0 quota=0` / 权重合计 100；API-DB-页面一致无缓存（`availableDates=['2026-07-10']`）；15:00–15:30 时段仅本 slot 无邻近 cron 冲突；DB 仅 1 行无重复；PM2 web/cron online（uptime 21h）；Health CRITICAL=0。**待 07-13 15:20 JST 复验**：cron 准时触发 + 脚本放行生成 07-13 行（15:18 前）+ 当日实时覆盖率与 GPT 无 429/quota → 出完整 PASS。
