// ── Deep Research V2 · AI Data Center Benchmark（P18-T2 · 真实执行）──────────
// AI 数据中心 V1(PUBLISHED 种子) → Strong Model(gpt-5.6-sol) → V2 候选(AI_RESEARCHED,
//   payload 入库不 apply, V1 保留) → 13 项质量报告 + 真实性字段 → Review → 人工 Approve → PUBLISHED。
// 复用与 AI 半导体完全相同的引擎/Provider/Post-Processor/Canonical/Version 系统，仅 KEY 不同。
// 禁批量八产业/禁自动 Approve/禁自动 Publish。
// 用法:
//   npx tsx scripts/research/benchmark-v2-datacenter.ts --persist    # 真实生成 + 入库 V2 候选
//   npx tsx scripts/research/benchmark-v2-datacenter.ts              # 生成 + 报告（不入库）
//   npx tsx scripts/research/benchmark-v2-datacenter.ts --mock --dry # 种子占位验证管线（无 key）
...
async function main() {
  ...
  console.log("═══ Deep Research V2 · AI Data Center Benchmark（真实执行）═══\n");
  ...
  if (!before.exists || before.status !== "PUBLISHED") { console.log(`⛔ V1 非 PUBLISHED（status=${before.status}），停止。先跑 run-industry.ts ${KEY} 建种子 V1。`); process.exit(2); }
  ...
  // 归一化：候选固定绑定到 AI_DATACENTER（模型可能省略/改写 industryKey）；缺名回填种子。不改研究内容。
...
main().catch((e) => { console.error("benchmark-v2-datacenter 失败:", e); process.exit(1); });
